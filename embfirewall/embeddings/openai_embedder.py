...
def _build_token_counter(model_id: str) -> Callable[[str], int]:
    """
    Best-effort token estimator for TPM throttling.
...
class _FixedWindowTokenLimiter:
    """
    Simple fixed-window token-per-window limiter.
    Not perfectly smooth, but deterministic and cheap.
    """
...
class OpenAICachedEmbedder(CachedEmbedder):
    ...
    def _encode_batch(self, texts: Sequence[str]) -> np.ndarray:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if arr.shape[0] != len(inp):
                    raise RuntimeError(f"OpenAI returned n={arr.shape[0]} embeddings for input n={len(inp)}")
                return arr
        ...
        raise RuntimeError(f"OpenAI embeddings failed after {self.max_retries} retries: {last_err}")
